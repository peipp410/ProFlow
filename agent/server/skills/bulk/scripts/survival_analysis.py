...
print(f"Here is the survival analysis report for the TCGA-{cancer_type} cancer type:")
...
for cluster, (p, m1, m2, trend) in pvals.items():
    if p < 0.05:
        print(f"Cluster {cluster}: p = {p:.4g}, survival trend is {trend}")
        sub = df_all[df_all['cluster'] == cluster]
    
        if sub.empty:
            print(f"Cluster {cluster} has no significant protein markers")
        else:
            sub = sub.sort_values('pvals_adj', ascending=True)
            top = sub.head(20)
            
            genes = top['gene'].tolist()
            print(f"Differentially expressed proteins in Cluster {cluster}: {','.join(genes)}")